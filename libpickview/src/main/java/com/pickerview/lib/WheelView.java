...
public class WheelView extends View {
	...
	// 条目间距倍数
	private static final float lineSpacingMultiplier = 1.4F;
	boolean isLoop;
	...
	// 可见的item数组
    private final Object[] visibles = new Object[itemsVisible];
	...
	final void scrollBy(float velocityY) {
		cancelFuture();
	...
	protected void onDraw(Canvas canvas)
	{
		...
		{
			Log.i(TAG, "出错了！adapter.getItemsCount() == 0，联动数据不匹配");
		}
		if (!isLoop)
		{// 不循环的情况
